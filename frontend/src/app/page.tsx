...
export default function ChatUI() {
  // Gracefully destructure loadThread (it will be undefined until we update the hook)
  const { messages, sendMessage, isStreaming, loadThread } = useChatStream() as any; 
  const [input, setInput] = useState('');
  const messagesEndRef = useRef<HTMLDivElement>(null);

  // [PHASE 5] Sidebar State
    ...
    <div className="flex h-screen w-full bg-white">
      ...
      <aside className="w-64 bg-gray-100 border-r border-gray-200 flex flex-col hidden md:flex">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {threads.length === 0 ? (
            <p className="text-sm text-gray-500 p-2 text-center mt-4">No history yet.</p>
          ) : (
            threads.map(threadId => (
              <button 
                key={threadId}
                className="w-full text-left p-3 rounded-lg text-sm text-gray-700 hover:bg-gray-200 transition-colors truncate border border-transparent hover:border-gray-300"
                onClick={() => {
                  if (loadThread) {
                    loadThread(threadId);
                  } else {
                    console.log("TODO: Update useChatStream hook to support loadThread for", threadId);
                  }
                }}
              >
                Session: {threadId.replace('thread_', '')}
              </button>
            ))
          )}
        </div>
