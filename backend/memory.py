...
async def save_thread_title(async_pool: AsyncConnectionPool, thread_id: str, user_message: str):
    """Generates and saves a title for a new thread if it doesn't exist."""
    async with async_pool.connection() as conn:
        async with conn.cursor() as cur:
            await cur.execute("SELECT title FROM thread_metadata WHERE thread_id = %s", (thread_id,))
...
async def get_all_threads_history(async_pool: AsyncConnectionPool):
    """Fetches all unique thread IDs joined with their titles."""
    async with async_pool.connection() as conn:
        async with conn.cursor() as cur:
            # [GOLDEN RULE] LEFT JOIN ensures backward compatibility for older threads without titles
            await cur.execute("""
